...
@CustomEditor (ColorCorrectionCurvesZSL)

class ColorCorrectionCurvesEditorZSL extends Editor {	
	...
	function ApplyCurves () {
   		if (applyCurveChanges) {
   			serObj.ApplyModifiedProperties ();   
			(serObj.targetObject as ColorCorrectionCurvesZSL).gameObject.SendMessage ("UpdateTextures");
       }   	
	}

	function ExportTex() {
		if(serObj == null) return;
		var cur : ColorCorrectionCurvesZSL = serObj.targetObject as ColorCorrectionCurvesZSL;
		cur.SaveFinalTex();
	}
