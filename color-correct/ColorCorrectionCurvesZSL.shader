Shader "ZSL/PostProc/ColorCorrectionCurves" {
	Properties {
	...
	half4 frag(v2f i) : COLOR 
	{
		...
		//color = half4(red.r, green.g, blue.b, color.a);
	...
	// 	half red = tex2D(_RgbTex, half2(color.r, ycoords.x)).r;//gb * half3(1,0,0);
	// 	half green = tex2D(_RgbTex, half2(color.g, ycoords.y)).g;//b * half3(0,1,0);
	// 	half blue = tex2D(_RgbTex, half2(color.b, ycoords.z)).b;// * half3(0,0,1);
		
	// 	half theDepth = UNITY_SAMPLE_DEPTH( tex2D (_CameraDepthTexture, i.uv2) );
	// 	half zval = tex2D(_ZCurve, half2( Linear01Depth (theDepth), 0.5));
		
	// 	half depthRed = tex2D(_RgbDepthTex, half2(color.r, ycoords.x)).r;//gb * half3(1,0,0);
	// 	half depthGreen = tex2D(_RgbDepthTex, half2(color.g, ycoords.y)).g;//b * half3(0,1,0);
	// 	half depthBlue = tex2D(_RgbDepthTex, half2(color.b, ycoords.z)).b;// * half3(0,0,1);
		
	// 	//color = half4( lerp(half3(red,green,blue), half3(depthRed,depthBlue,depthGreen), zval), color.a);
